...
@blueprint.route('/view_expenses', methods=['GET', 'POST'])
def view_expenses():
    login_form = LoginForm(request.form)
    if not current_user.is_authenticated:
        return render_template('accounts/login.html',
    ...
    expenses = get_expenses_fortbl_by_user_id(current_user)
    #return redirect(url_for('home_blueprint.view_expenses'))
